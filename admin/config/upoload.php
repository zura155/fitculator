...
if (!isset($_SESSION['username']) || $_SESSION['username'] == '' || $num_rows != 1){
	...
	$sfero_geo = isset($_POST['sfero_geo']) ? $_POST['sfero_geo'] : '';
	$sfero_eng = isset($_POST['sfero_eng']) ? $_POST['sfero_eng'] : '';
	...
	if (isset($_FILES["images"]))
		...
		$sql="INSERT INTO `designers` (`id`, `name_geo`, `name_eng`, `sfero_geo`, `sfero_eng`, `fb_link`, `in_link`, `photo`) VALUES (NULL, '".mysql_real_escape_string($name_geo)."', '".mysql_real_escape_string($name_eng)."', '".mysql_real_escape_string($sfero_geo)."', '".mysql_real_escape_string($sfero_eng)."', '".mysql_real_escape_string($fb_link)."', '".mysql_real_escape_string($in_link)."', '".mysql_real_escape_string($logo_name)."')";
