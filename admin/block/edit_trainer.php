...
if (!isset($_SESSION['username']) || $_SESSION['username'] == '' || $num_rows != 1){
	...
	exit(0);
		...
		if(!empty($_POST))
		{
			...
			$error = mysql_query("UPDATE `trainers` SET `name_geo` = '".$name_geo."', `name_eng` = '".$name_eng."', `fb_link` = '".$fb_link."', `in_link` = '".$in_link."', `photo` = '".$logo_name."' WHERE `trainers`.`id` = ".$designerId.";");
