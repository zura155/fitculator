...
if (!isset($_SESSION['username']) || $_SESSION['username'] == '' || $num_rows != 1){
	...
	exit(0);
		...
		if(!empty($_POST))
		{
			...
			$sfero_geo = $_POST['sfero_geo'];
			$sfero_eng = $_POST['sfero_eng'];
			...
			$error = mysql_query("UPDATE `advisor` SET `name_geo` = '".$name_geo."', `name_eng` = '".$name_eng."', `sfero_geo` = '".$sfero_geo."', `sfero_eng` = '".$sfero_eng."', `fb_link` = '".$fb_link."', `in_link` = '".$in_link."', `photo` = '".$logo_name."' WHERE `advisor`.`id` = ".$designerId.";");
