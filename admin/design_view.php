        ...
        <nav class="navbar navbar-transparent navbar-absolute">
            <div class="container-fluid">
                ...
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#">დიზაინები</a>
                </div>
                    ...
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header" data-background-color="purple">
                                ...
                                <h4 class="title">დიზაინები</h4>
                                <p class="category">ჩვენი ყველა დიზაინი</p>
                            </div>
                            <div class="card-content table-responsive">
                                <table class="table">
                                    <thead class="text-primary">
                                        <th>ID</th>
                                        <th>დიზაინის სახელი</th>
                                        <th>დიზაინის აღწერა</th>
                                    </thead>
                                    <tbody>
                                        ...
                                        $stmt = $db->query('SELECT count(id) FROM designes');
                                        ...
                                        $data = $db->query('SELECT * FROM designes ORDER BY id DESC '.$pages->get_limit());
                                        foreach($data as $news_row) {
                                        //display the records here


                                        	$imagesSql = mysql_query("SELECT * FROM images WHERE `table_relative` = 'designes' and `produc_id` = ".$news_row['id']."");
											$image1Row = mysql_fetch_assoc($imagesSql);
                                        ?>
                                    
                                        <tr>
                                                <td><a href="?<?php echo $news_row["id"]; ?>"><img src="../upload/designes/<?php echo $image1Row["image_path"]; ?>" style="width: 70px; min-height:50px; max-height:50px;"></a></td>
                                                <td><?php echo $news_row["designe_title_geo"]; ?></td>
                                                <td><?php echo $news_row["designe_description_short_geo"]; ?></td>


                                            <td class="td-actions text-right">

                                                <form method="post" action="edit_design.php">
                                                	<input type="hidden" name="data_id" value="<?php echo $news_row["id"]; ?>">
	 ...
	 <script>
		$(document).ready(function() {
			...
			$("#confirm-submit .btn-success.success").click(function(){
				...
				$.post('', {table_name: 'designes', data_id: currentId});
				location.reload();	
			});
